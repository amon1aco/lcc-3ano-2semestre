...
void renderCatmullRomCurve() {
	glBegin(GL_LINE_STRIP); // Use GL_LINE_STRIP para desenhar a curva suavemente
	for (float t = 0; t <= 1; t += 0.02) {
		float pos[3];
		float deriv[3];
		getGlobalCatmullRomPoint(t, pos, deriv);
		glVertex3f(pos[0], pos[1], pos[2]); // Desenha o ponto intermediário

		// Desenhar a tangente
		float tangentEndPoint[3];
		for (int i = 0; i < 3; ++i) {
			tangentEndPoint[i] = pos[i] + deriv[i]; // Ponto da tangente é o ponto atual mais a derivada
		}
		glVertex3f(tangentEndPoint[0], tangentEndPoint[1], tangentEndPoint[2]); // Desenha a linha da tangente
	}
	glEnd();
}
