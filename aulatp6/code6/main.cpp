...
void terrainVBO(float altura, float largura) { // altura não corresponde ao eixo dos y
    float* vertexB = NULL, altura_aux = altura, largura_aux;
    int coluna, linha, i = 0;

    float altura_step = altura / th;
    float largura_step = largura / tw;
    ...
    for (linha = 0; linha < th - 1; linha++) {
        ...
        for (coluna = 0; coluna < tw; coluna++) {
            ...
            vertexB[i++] = altura_aux - altura_step;
            vertexB[i++] = h(coluna, linha + 1);
            vertexB[i++] = largura_aux;

            largura_aux += largura_step;
        }
        altura_aux -= altura_step;
    }
...
void carregarImagem() {
    ...
    std::cout << "Width: " << tw << std::endl;

    th = ilGetInteger(IL_IMAGE_HEIGHT);
    std::cout << "Height: " << th << std::endl;
